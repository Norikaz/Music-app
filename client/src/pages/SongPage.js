...
export const SongPage = (props) => {
    ...
    useEffect(() => {
        axios(
            `https://api.spotify.com/v1/search?q=track:${songName}&type=track`,
            {
                headers: {
    ...
    return (
        <div>
            ...
            <SongInfo
                ...
                albumCover={
                    spotifySongInfo
                        ? `${spotifySongInfo.album.images[1].url}`
                        : ""
                }
                artistName={
                    spotifySongInfo
                        ? spotifySongInfo.artists.map((artist) => (
                              <p>{artist.name}</p>
                          ))
                        : "N/A"
                }
                artistImage={
                    spotifyArtistInfo ? spotifyArtistInfo.images[2].url : ""
                }
                genres={
                    spotifyArtistInfo
                        ? spotifyArtistInfo.genres.map((genre) => (
                              <p>{genre}</p>
                          ))
                        : "N/A"
                }
